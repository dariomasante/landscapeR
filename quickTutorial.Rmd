...
All basic GIS operations are handled by the [`raster`](https://cran.r-project.org/web/packages/raster/index.html) package.
...
```{r}
patchSize = sample(10:50, 10)
newVal = 5
rr = makeClass(rr, 10, size=patchSize, val=newVal, rast=TRUE, bgr=1)
plot(rr)
```

Now let's add a particular feature (could be a lake) and build a stream out of it:
```{r, warning=FALSE}
rr = makePatch(rr, 30, rast=TRUE, val=2, bgr=1)
rr = expandClass(rr, 2, 50)
plot(rr)
```
