...
###
#rmSingle0 <- function(rst, rm = TRUE){
#   dim1 <- dim(rst)[1]
#   dim2 <- dim(rst)[2]
#   v <- vval <- raster::getValues(rst)
#   v <- which(!is.na(v))
#   singles <- lapply(v, function(x) {
#     if(x %% dim2 == 0){
#       cc <- dim2
#       rr <- x / dim2
#     } else {
...
#     ad <- ad[5:8] + (ad[1:4]-1) * dim2
#     ad <- ad[!is.na(ad)]
#     if(all(.subset(vval, ad) != .subset(vval, x))){
#       if(rm == TRUE){
#         sample(.subset(vval, ad), 1)
#       } else {
#         x
#       }
#     }
...
#     return(rst)
#   } else {
#     return(unlist(singles))
#   }
# }
###
#rmSingle1 <- function(rst, rm = TRUE){
#   dim1 <- dim(rst)[1]
#   dim2 <- dim(rst)[2]
#   v <- vval <- raster::getValues(rst)
#   v <- which(!is.na(v))
#   singles <- vapply(v, function(x) {
#     if(x %% dim2 == 0){
#       cc <- dim2
#       rr <- x / dim2
#     } else {
...
#     return(rst)
#   } else {
#     return(singles[!is.na(singles)])
#   }
# }

###
#rmSingle2 <- function(rst, rm = TRUE){
#   dim1 <- dim(rst)[1]
#   dim2 <- dim(rst)[2]
#   v <- vval <- raster::getValues(rst)
#   v <- which(!is.na(v))
